# Christian Tonny

Software engineer in Rwanda — building local-first AI tools, Claude skills, and developer products worth starring.

## Building now

| Project | What it is |
|---------|------------|
| [**Daylens**](https://github.com/irachrist1/daylens) | Local-first activity timeline for your laptop — ask AI what you worked on, zero cloud |
| [**daylens-mcp**](https://github.com/irachrist1/daylens-mcp) | MCP server — plug your Daylens history into Cursor and Claude Code |
| [**TARS**](https://github.com/irachrist1/tars) | Chief of Staff skill for Claude + Microsoft 365 — reads your files, drafts from precedent |
| [**claude-code-workflow-os**](https://github.com/irachrist1/claude-code-workflow-os) | Claude Code plugins for project memory, handoffs, and ship checks |
| [**tidytabs**](https://github.com/irachrist1/tidytabs) | One-click Chrome tab grouping — preview before you apply |
| [**homebrew-daylens**](https://github.com/irachrist1/homebrew-daylens) | `brew install --cask daylens` — official macOS tap |

## Elsewhere

**[christian-tonny.dev](https://christian-tonny.dev)** · [LinkedIn](https://linkedin.com/in/irachrist1) · mail@christian-tonny.dev
